...
router.get("/client/detail", async (req, res) => {
    try {
        ...
        await Client.insertMany(clientdata.map(client => ({
            name: client.name,
            email: client.email,
...
router.get("/supervisor/details", async (req, res) => {
    try {
        ...
        await Supervisor.insertMany(supervisors.map(supervisor => ({
...
router.get("/getExpense/detail", async (req, res) => {
    ...
    const pendingexpense = expenseDetails.filter(expense => 
...
router.post("/expenses/approve", async (req, res) => {
    ...
    try{
        ...
        if(status === "approved"){
            ...
            const clientData=await Client.findOne({_id:supervisor.allorted_client});
